(function ($) {
    ...
    $.hDialog.defaults = $.extend({}, $.fn.dialog.defaults, {
        ...
        onMove: function (left, right) {
            ...
            var dialogwidth = $("#SearchID").width();
            var dialogHeight = $("#SearchID").height();
            var diaH = dialogHeight - 50;
            if (bodyHeight < dialogHeight || bodyWidth < bodyWidth || (bodyHeight < dialogHeight && bodyWidth < bodyWidth)) { return; };

            if (left < 0) {
                $("#SearchID").dialog('move', { left: 0, top: right });
            } else if ((left + dialogwidth) > (bodyWidth - 50)) {
                rightTwo = bodyWidth - dialogwidth - 50;
                $("#SearchID").dialog('move', { left: rightTwo, top: right });
            };
            if (right < 0) {
                $("#SearchID").dialog('move', { left: left, top: 0 });
            } else if (right > (bodyHeight - diaH)) {   //if (top > (bodyHeight - bodyHeight - dialogHeight - 50))与 $('#Div1').dialog('move', { left: left, top: bottom });共存出错
                bottom = bodyHeight - dialogHeight - 50;
                $("#SearchID").dialog('move', { left: left, top: bottom });
            };
        }
